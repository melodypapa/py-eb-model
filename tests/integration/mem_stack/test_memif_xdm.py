"""Integration test for MemIf.xdm parsing.

Implements: SWR_INTEGRATION_00004 (MemIf parsing)
"""
import os
import pytest
...
XDM_PATH = "simple_demo_mem_can_rte/config/MemIf.xdm"


@pytest.mark.integration
def test_memif_simple_mem_can_xdm_parsing():
    """Test MemIf.xdm parsing from simple_demo_mem_can_rte."""
    ...
    assert module_name == "MemIf", f"Expected module MemIf but got {module_name}"
    ...
    module = doc.find("/MemIf/MemIf")
    assert module is not None, f"Module MemIf not found in EBModel after parsing {XDM_PATH}"
